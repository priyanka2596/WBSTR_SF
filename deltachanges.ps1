...
git diff --name-only HEAD^ HEAD > lastcommitchanges.txt
...
echo $path
	...
	#source files
	$s1=@()
$Files =Get-Content -path "$ENV:WORKSPACE\lastcommitchanges.txt" #Give the file path which has Github's latest file list
	write-host $Files
